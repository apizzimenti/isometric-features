 ...
 *
 * @param {string} element                  String id for the HTML element that will serve as your guide..
 * @param {string} gameElement              String id for element that hosts the game canvas (the same name as when the Phaser game is initialized).
 * @param {object} [buttonOptions=null]     Button text and style options.
 * @param {string} [buttonOptions.text="i"] Text displayed within the button.
 * @param {object} [buttonOptions.style={}] Styling for guide button.
 * @param {object} [buttonOptions.hover={}] Styling for guide button.
 * @param {object} [menuStyles=null]        Custom style overrides.
 *
 * @example
 *
 * // index.html
...
function Guide (element, gameElement, buttonOptions, menuStyles) {
    ...
    if (!Globals.paramNotExist(buttonOptions, "object")) {
...
Guide.prototype._configureWindow = function () {
    ...
    // enable using esc to get out of the menu
    $("body").keydown((e) => {
        if (e.which === 27) {
            _this.raw.guideButton.on = false;
            $(`#${_this.raw.guideId}`).css({
