...
Mouse.prototype.selected = function (callback, args, animation) {
        ...
        this.group.forEach((tile) => {
            ...
            if (tile.type === "tile" && !callback) {
                ...
                if (inside) {
                    ...
                    tile.tint = animation.tint || 0x98FB98;
                    tile.alpha = animation.alpha || 1.3 + Math.sin(this.game.time.now * 0.007);
                    
                    this.tween = this.game.add.tween(tile).to(...animation.tween || {isoZ: 5}, 20, Phaser.Easing.Linear.None, true);
                    this.tweened = true;
