...
Debug.prototype.sprite = function (sprites) {
    ...
    if (this.on) {
        var debugSprite = (sprite) => {
            this.game.debug.text(`${sprite.sprite.row}, ${sprite.sprite.col}`, this.x, this.y, this.color);
            
            try {
                this.game.debug.body(sprite.sprite.tile.top, "#FF0000", false);
                this.game.debug.body(sprite.sprite, "#FFFFFF", false);
    
                for (var tile in sprite.sprite.tile) {
                    if (sprite.tile.hasOwnProperty(tile) && tile !== "top") {
                        this.game.debug.body(sprite.tile[tile], "#FFDD00", false);
                    }
                }
            } catch (e) {
